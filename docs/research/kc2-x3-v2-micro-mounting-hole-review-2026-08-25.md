...
The selected digital implementation candidate is:
...
- 2.00 mm maximum screw-head diameter and 0.50 mm maximum head height;
...
The holes are not a substitute for the existing load path. The perimeter rail and the existing 14 left / 11 right distributed supports continue to carry key loads into the desk contacts. The new mounting points are intended to clamp the PCB onto those coplanar supports, provisionally locate it in XY, prevent anti-lift motion, and add local zero-gap support/desk-contact columns. Actual full-pattern XY registration remains pending until the printed-fit coupon proves that the round-hole pattern seats without binding.
...
- NBK `SNZS-M1.4-3` proves that a catalogued M1.4 x 3 precision machine screw exists with 0.30 mm pitch and a 2.00 mm diameter by 0.50 mm high PH0 head. It is a geometry reference, not proof of stock, procurement, or a direct-plastic joint.
...
- A 1.60 mm NPTH is routine and a 1.60 mm-wide NPTH relief slot is manufacturable, but the 2.00 mm screw head leaves too little bearing width for a long PCB slot. The first digital implementation therefore keeps round 1.60 mm holes and treats multi-axis fit as a physical coupon gate.
...
Coordinates are KiCad board coordinates in millimetres. References shall be `MH1...MH8` on the left and `MH1...MH10` on the right; legacy `H*` and `REG*` references remain forbidden.
...
## Verified Digital Search Margins
...
## Housing Stack For The Digital Prototype
...
- Each mounting point also extends to the Z = -0.80 mm desk datum as a local support foot so a clamped PCB load is not carried by a floating 2.50 mm plate span.
- Initial direct-thread coupon pilot assumption: 1.10 mm nominal diameter and 2.80 mm nominal blind depth from the housing top, extending 0.30 mm into the local 0.80 mm desk-contact column and leaving a 0.50 mm nominal closed bottom.
...
- All PCB holes are initially 1.60 mm round NPTHs. This avoids the inadequate bearing width of a 1.60 mm by 2.00+ mm relief slot under a 2.00 mm head.
...
Proceed with the M1.4 8-left / 10-right digital geometry and preserve all existing distributed supports. Treat the 1.10 mm blind pilot and the low-head M1.4 fastener as a prototype interface, not a procurement-approved fastening system. Do not claim physical retention, deflection compliance, repeat serviceability, fabrication readiness, or order readiness until the coupon gates above pass.

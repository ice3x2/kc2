...
상태: 구현 및 검증 진행 중. 현재 `hardware/MODELS`의 게시된 가림벽 보강본을 새 충전형으로 교체하지 않았습니다.
...
`tools/fusion/KC2FilledPlatesToF3D/KC2FilledPlatesToF3D.py`로 여섯 STEP을 실제 Fusion에서 열고 F3D를 저장한 뒤 다시 열어 STEP으로 읽어내는 작업은 모두 통과했습니다. 원본 STEP 여섯 개의 전체 단면 검사도 통과했습니다. 읽어낸 native 형상은 `tools/review_kc2_filled_native.py`로 독립 단면 계약과 대조하는 중입니다. 단순 파일 존재나 Fusion 체적 일치만으로 완료하지 않습니다. 여섯 native 읽기 결과의 실제 형상 검사, 최종 게시 검증 및 현재 인쇄 안내 갱신은 아직 남아 있습니다. 현재 보고서의 임시 경로 바인딩은 개발 환경의 관측 기록이며, 새 체크아웃에서의 독립 게시 검증을 아직 주장하지 않습니다.
...
저상형 보스는 판보다 조금 높습니다. 실제 키캡 내부·전체 이동·체결 강도는 미검증이며, 기존 MX 나사를 저상형에 사용하지 않습니다. PCB/거버와 하판 두 옵션은 변경하지 않습니다. 전체 목표는 아직 완료되지 않았습니다.

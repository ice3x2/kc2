...
STEP_CODE=('tools/kc2_step_whitespace.py','tools/test_kc2_step_whitespace.py',
           'tools/test_kc2_registered_step_publication.py','tools/publish_kc2_registered_housings.py')


def step_sources():
    return {source:dict(target=target,raw_destination=REPORT+'/evidence/raw-step/'+Path(target).name+'.raw')
            for target,source in inventory().items() if target.endswith('.step')}


def step_publication(read):
    """Only lexical formatting changes; original CAD audit bytes remain raw."""
    from tools.kc2_step_whitespace import normalize
    code={name:sha_bytes(read(name)) for name in STEP_CODE};rows={}
    for source,mapping in step_sources().items():
        _,record=normalize(read(source))
        rows[source]={**mapping,**record,'code_sha256':code}
    return rows


def check_step_records(records,read):
    expected=step_publication(read)
    if json_bytes(records)!=json_bytes(expected):raise ValueError('STEP normalization identity, mapping or lexical evidence differs')
    return expected


def verify_step_publication(root,manifest):
    from tools.kc2_step_whitespace import normalize
    root=Path(root).resolve()
    def read(name):
        return portable_read(root,manifest,name) if name in step_sources() else contained(root,name).read_bytes()
    rows=check_step_records(manifest.get('step_normalization'),read)
    for source,row in rows.items():
        output=contained(root,row['target']).read_bytes();expected,_=normalize(read(source))
        if output!=expected or manifest.get('outputs',{}).get(row['target'])!=row['normalized_sha256']:
            raise ValueError('Published STEP differs from authenticated lexical-only normalization')
...
def destination(source):
    ...
    if source in step_sources():return step_sources()[source]['raw_destination']
...
def preflight(root,*,_manifest=None):
    ...
    normalization={}
    try:
        normalization=step_publication(read)
        for name in STEP_CODE:
            bindings[name]=dict(kind='workspace',path=name,sha256=sha_bytes(read(name)),destination=destination(name))
        if _manifest is not None:
            if _manifest.get('step_normalization')!=normalization:raise ValueError('Portable STEP normalization record differs')
            verify_step_publication(root,_manifest)
    except (OSError,ValueError,KeyError,TypeError) as exc:errors.append('STEP normalization: '+str(exc))
...
def verify_bytes(root,manifest):
    ...
    try:
        ...
        verify_step_publication(root,manifest)
...
def portable_read(root,manifest,logical):
    ...
    if logical in reverse and logical not in step_sources():
...
def publish(root):
    ...
    normalization=check_step_records(gate.get('step_normalization'),lambda name:contained(root,name).read_bytes())
    for target,source in inventory().items():
        data=contained(root,source).read_bytes()
        if sha_bytes(data)!=expected[target]:raise ValueError('Staged output changed after preflight')
        if source in normalization:
            from tools.kc2_step_whitespace import normalize
            data,record=normalize(data)
            if record!={k:v for k,v in normalization[source].items() if k not in ('target','raw_destination','code_sha256')}:
                raise ValueError('STEP formatting changed after preflight')
        outputs[target]=sha_bytes(data);payload[target]=data

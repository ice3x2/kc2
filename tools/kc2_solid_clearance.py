...
def _validated_pair(a,b,aa,bb,distance_factory=None):
    ...
    # The two-shape constructor performs automatically; use the empty form to
    # avoid a duplicate solve and enable native parallelism before the one run.
    solver=distance_factory()
    solver.LoadS1(a.wrapped)
    solver.LoadS2(b.wrapped)
    solver.SetMultiThread(True)
    solver.Perform()

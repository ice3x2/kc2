...
class SnapshotTests(unittest.TestCase):
    def test_export_filter_writes_only_verified_exact_zero_removal(self):
        import trimesh
        from tools.test_kc2_stl_zero_area_filter import append
        good=trimesh.creation.box().export(file_type='stl')
        bad=append(good,[[[.5,.5,.5],[.5,.5,.5],[-.5,.5,.5]]]*6)
        with tempfile.TemporaryDirectory() as tmp:
            path=Path(tmp)/'part.stl';path.write_bytes(bad)
            record=stage.normalize_exported_stl(path)
            self.assertEqual(path.read_bytes(),good)
            self.assertEqual(record['removed_count'],6)
            self.assertEqual(record['output_sha256'],stage.digest(path))
            broken=b'invalid STL';path.write_bytes(broken)
            with self.assertRaises(ValueError):stage.normalize_exported_stl(path)
